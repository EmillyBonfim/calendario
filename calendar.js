...
function colorirDia() {
    ...
    if (parseInt(days) > 30) {
        alert("Esse dia é inválido. O número indicado não pode ser maior do que 30.");
    ...
    if (index >= 0 && index < tds.length) {
        ...
        if (colorCounts[color] >= 3) {
            alert('Nossos veículos só transportam 3 vezes por mês! Por favor selecione outro veículo.');
        ...
        alert('Dia selecionado é inválido.');
    }
}